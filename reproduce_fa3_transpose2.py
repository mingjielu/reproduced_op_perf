...
# normal fa3 kernel_func 20ms on mi308x
with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], with_stack=True) as prof:
    aa = flash_attn_func(q,k,v,causal=True) #bshd
    aa.sum().backward()
print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))


# cannot call fa3: 52ms on mi308x
my_linear = nn.Linear(4096,4096,dtype=torch.bfloat16).to('cuda')
with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], with_stack=True) as prof:
    aa = flash_attn_func(q,k,v,causal=True) #bshd
    aa.retain_grad()
    aa.register_hook(hook_fn)
    #bb = aa.transpose(0,1).contiguous() # it doesn't matter whether bb is contiguous 
    bb = aa.transpose(0,1)  # it doesn't matter whether bb is contiguous
    cc = bb.reshape(s,b,-1) # s, b, hd
    dd = my_linear(cc)
    dd.sum().backward()
